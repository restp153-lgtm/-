...
############################################################
# 2. 使用 caret 進行 Random Forest 調參
############################################################
set.seed(123)
...
# tuning 欄位：
# mtry: 每棵樹抽取的變數數量
# splitrule: 分割規則（rf 使用 "gini"）
# min.node.size: 葉節點最小樣本數
...
rf_tuned <- train(
  WL_num ~ .,
  data = rf_data,
  method = "ranger",          # 比 randomForest 更快
  trControl = control,
  tuneGrid = tune_grid,
  importance = "impurity"
)
...
cat("\n最佳 Random Forest 準確率：",
    max(rf_tuned$results$Accuracy), "\n")
...
############################################################
# 4. 預測勝率（P(win) = class "1"）
############################################################
rf_pred_prob_full <- predict(rf_final, rf_data)$predictions[, "1"]

GameLogs2$rf_pred_prob <- rf_pred_prob_full
GameLogs2$rf_pred_class <- ifelse(rf_pred_prob_full > 0.5, 1, 0)

accuracy_rf_full <- mean(GameLogs2$rf_pred_class == GameLogs2$WL_num)
cat("最終模型逐場準確率：", round(accuracy_rf_full, 4), "\n")
...
team_summary_rf <- GameLogs2 %>%
  group_by(TEAM_ABBREVIATION) %>%
  summarise(
    avg_pred_prob   = mean(rf_pred_prob),
    actual_win_rate = mean(WL_num),
    .groups = "drop"
  )

ggplot(team_summary_rf,
       aes(x = actual_win_rate, y = avg_pred_prob, label = TEAM_ABBREVIATION)) +
  geom_point(color = "purple", size = 3) +
  geom_text(vjust = -0.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  labs(
    title = "最佳化 Random Forest 預測勝率 vs 真實勝率",
    x = "真實勝率",
    y = "預測勝率"
  ) +
  theme_minimal()
